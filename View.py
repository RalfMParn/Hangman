...
import time
from PIL import ImageTk, Image  # "pip install Pillow" in Terminal (upgrade if you want)
...
class View(Tk):
    def __init__(self, controller, model):
        ...
        # Kirjastiilid
        self.__big_font = font.Font(family='Courier New', size=20, weight='bold')
        ...
        self.__lbl_error, self.__lbl_time, self.__lbl_result = self.create_lables()
        # Loome sisestus kasti (Näide kudias veel teha, kui on 'vähe' asju)
    ...
    def create_frames(self):
        top = Frame(self, height=50, bg="black")
        bottom = Frame(self)
    ...
    def draw_scoreboard(self, frame, data):
        if len(data) > 0:
            ...
            my_table.heading('name', text='Mängija', anchor=CENTER)
            ...
            for p in data:
                ...
                my_table.insert(parent='', index='end', iid=str(x),
                                text='', values=(p.name, p.word, p.missing, sec, dt))
                x += 1
    ...
    def ask_usr_name(self):
        user_name = simpledialog.askstring("Nimi", "Mis on sinu Nimi?")
        return user_name
